...
from modules.networks import KPDetectorConfidence
...
from datasets.annot_converter import HUMANS_TO_MPII, HUMANS_TO_LSP, HUMANS_TO_PENN, MPII_TO_PENN
from confidence_trainer import train_kpdetector
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tgt_dataset", default="mpii", type=str, help="which dataset to use for evaluation. [h36m | penn]")
    ...
    model_kp_confidence = KPDetectorConfidence(model_kp_detector, config['model_params']['kp_detector_params']['num_kp'])
    ...
    if opt.dataset == "humans":
        ...
        #if opt.tgt_dataset != opt.dataset:
        loader_tgt = LoadMpii(**config['datasets'][dataset_eval]) 
    
    loader_third = None
    if opt.tgt_dataset != opt.dataset:
        dataset_eval = 'penn_test'
        loader_third = LoadPennAction(**config['datasets'][dataset_eval]) 
        kp_map = MPII_TO_PENN
    ...
    train_kpdetector(model_kp_confidence,
                       loader,
                       loader_tgt,
                       config['train_params'],
                       opt.checkpoint,
                       logger, opt.device_ids, tgt_batch, pretrained=True, kp_map=kp_map, loader_third=loader_third)
